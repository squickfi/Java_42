...
public class NumberWorkerTest {
    ...
    @ParameterizedTest
    @ValueSource(ints = {2, 3, 113, 11, 283})
    void isPrimeForPrimes(int number) {
        Assertions.assertTrue(numberWorker.isPrime(number));
    }

    @ParameterizedTest
    @ValueSource(ints = {42, 55, 100, 117, 1024})
    ...
    void testDigitSum(int number, int expected) {
        Assertions.assertEquals(numberWorker.digitSums(number), expected);
    }
}
